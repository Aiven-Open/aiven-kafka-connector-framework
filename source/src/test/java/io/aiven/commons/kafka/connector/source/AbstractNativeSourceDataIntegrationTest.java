...
public abstract class AbstractNativeSourceDataIntegrationTest<K extends Comparable<K>, N> extends AbstractSourceIntegrationBase<K, N> {
    ...
    void getNativeItemStream() {
        ...
        final TreeSet<K> expected = new TreeSet<>(writeResults.stream().map(SourceStorage.WriteResult::getNativeKey).toList());
        List<K> actual = underTest.getNativeItemStream(null).map(AbstractSourceNativeInfo::nativeKey).toList();
        assertThat(actual).containsExactlyElementsOf(expected);

        // get an offset that is not the beginning.
        K startKey = expected.iterator().next();
        actual = underTest.getNativeItemStream(startKey).map(AbstractSourceNativeInfo::nativeKey).toList();
        // some clients will return the key others will return the first value larger than the key.
    ...
    void offsetManagerEntry() {
        ...
        OffsetManager.OffsetManagerEntry entry2 = underTest.createOffsetManagerEntry(entry1.getProperties());
    ...
    void getIterator() {
        ...
        Iterator<EvolvingSourceRecord> iter = underTest.getIterator(context -> true);// accept all records
        ...
        // calling getIterator again should only read the latest data and perhaps the last key.
