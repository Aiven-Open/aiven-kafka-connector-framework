...
public abstract class IORecordTransformerTest extends IOTransformerTest {
  ...
  /**
   * Test that invalid data does not throw an exception to the reader and does not return any daa.
   */
  @Test
  final void testReadRecordsInvalidData() throws IOException {
    final ExampleNativeItem nativeItem =
        new ExampleNativeItem(
            "nativeKey", ByteBuffer.wrap("A-bad-data-block".getBytes(StandardCharsets.UTF_8)));
  ...
  final void testReadRecordsSkipFew() throws Exception {
    ...
    final ExampleOffsetManagerEntry entry =
        (ExampleOffsetManagerEntry) evolvingSourceRecord.getOffsetManagerEntry();
