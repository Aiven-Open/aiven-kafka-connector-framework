...
public class ExampleCsvClient {
	...
	public ExampleCsvClient() throws IOException {
		...
		dataMap.put("second 10", CsvTestDataFixture.generateCsvRecords(10, 10, CsvTestDataFixture.TEST_MESSAGE));
		dataMap.put("third 10", CsvTestDataFixture.generateCsvRecords(20, 10, CsvTestDataFixture.TEST_MESSAGE));
	}
	 ...
	 *            {@link ExampleCsvSourceData} is a String
	 * @return the list of native objects. This is a collection of String because
	 *         that is the type of the N in {@code NativeSourceData<K,N,O,T>}
	 *
	 */
	Collection<ExampleNativeItem> listObjects(String offset) {
		...
		if (dataSent == null) {
			...
			return Arrays.asList(
					new ExampleNativeItem(dataMap.get("first 10"),
							dataMap.get("first 10").getBytes(StandardCharsets.UTF_8)),
					new ExampleNativeItem(dataMap.get("second 10"),
							dataMap.get("second 10").getBytes(StandardCharsets.UTF_8)));
		} else if (dataSent.equals("second 10")) {
			dataSent = "third 10";
			return Collections.singletonList(new ExampleNativeItem(dataMap.get("third 10"),
					dataMap.get("third 10").getBytes(StandardCharsets.UTF_8)));
		}
