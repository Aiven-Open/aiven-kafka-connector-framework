...
public class DebeziumLogicalConverters {
	...
	public static void initialize(final BigQuerySinkConfig config) {
		...
		LogicalConverterRegistry.registerIfAbsent(Timestamp.SCHEMA_NAME,
				new TimestampConverter(config.getShouldConvertDebeziumTimestampToInteger()));
		LogicalConverterRegistry.registerIfAbsent(VariableScaleDecimal.LOGICAL_NAME,
	...
	public static class MicroTimeConverter extends LogicalTypeConverter {
		...
		@Override
		public String convert(Object kafkaConnectObject) {
			// We want to maintain the micro second info, but date only supports up to
			// milli.
	...
	/**
	 * Class for converting Debezium micro timestamp logical types to BigQuery
	 * datetimes.
	 */
	...
	public static class TimestampConverter extends LogicalTypeConverter {
		...
		public TimestampConverter(boolean asInteger) {
			super(Timestamp.SCHEMA_NAME, Schema.Type.INT64,
	...
	/**
	 * Class for converting Debezium zoned timestamp logical types to BigQuery
	 * timestamps.
	 */
	public static class ZonedTimestampConverter extends LogicalTypeConverter {
		...
		public String convert(Object kafkaConnectObject) {
			...
			DateTimeFormatter bqZonedTimestampFormat = new DateTimeFormatterBuilder()
					.append(DateTimeFormatter.ISO_LOCAL_DATE).appendLiteral(' ').append(DateTimeFormatter.ISO_TIME)
					.toFormatter();
	...
	public static class VariableScaleDecimalConverter extends LogicalTypeConverter {
		...
		public Field.Builder getFieldBuilder(Schema schema, String fieldName,
			...
			switch (decimalHandlingMode) {
				case RECORD :
					return convertStruct.apply(schema, fieldName)
