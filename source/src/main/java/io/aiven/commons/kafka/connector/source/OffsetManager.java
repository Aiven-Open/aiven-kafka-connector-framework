...
public final class OffsetManager {
  ...
  /**
   * Get entry data from the offset manager. This method will return a copy of the local data if it
   * has been created otherwise will get the data from Kafka. If there is not a local copy and not
   * one from Kafka then an empty Optional is returned
   *
  ...
  public Optional<Map<String, Object>> getEntryData(OffsetManagerKey key) {
    final Map<String, Object> data =
        offsets.compute(
            ...
            (k, v) -> {
              if (v == null) {
                final Map<String, Object> kafkaData =
                    context.offsetStorageReader().offset(key.getPartitionMap());
   ...
   *
   * @param entry the entry that should be added to the offset manager.
   */
  ...
  /**
   * Gets any offset information stored in the offsetStorageReader and adds to the local offsets
   * Map. This provides a performance improvement over when checking if offsets exists individually.
   *
   * @param offsetManagerKeys A Collection of OffsetManagerKey which identify individual offset
   *     entries
   */
  public void populateOffsetManager(
      final Collection<OffsetManager.OffsetManagerKey> offsetManagerKeys) {
    final Map<Map<String, Object>, Map<String, Object>> matchingOffsets = new HashMap<>();
    context
        .offsetStorageReader()
        .offsets(
            offsetManagerKeys.stream().map(OffsetManagerKey::getPartitionMap).collect(toList()))
        .entrySet()
        .stream()
   ...
   * {@link SourceTaskContext}.
   *
   * @param sourceRecord the SourceRecord that contains the key to be removed.
   */
  ...
  public interface OffsetManagerEntry {
     ...
     * This method may throw a RuntimeException if required properties are not defined in the map.
     *
    ...
    /**
     * Gets the value of the named property. The value returned from a {@code null} key is
    ...
    /**
     * Sets a key/value pair. Will overwrite any existing value. Implementations of
     * OffsetManagerEntry may declare specific keys as restricted. These are generally keys that are
     * managed internally by the OffsetManagerEntry and may not be set except through provided
     * setter methods or the constructor.
     *
     ...
     *
     * <p>The return value should be a copy of the internal structure or constructed in such a way
     * that modification to the key values is not reflected in the OffsetManagerEntry.
     *
