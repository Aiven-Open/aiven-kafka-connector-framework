...
/**
 * Abstract class for logical type converters. Contains logic for both schema
...
public abstract class LogicalTypeConverter {
	 ...
	 * @param encodingType
	 *            The encoding type of the logical type.
	 * @param bqSchemaType
	 *            The corresponding BigQuery Schema type of the logical type.
	 */
	public LogicalTypeConverter(String logicalName, Schema.Type encodingType, LegacySQLTypeName bqSchemaType) {
	 ...
	 *             if the given schema encoding type is not the same as the expected
	 *             encoding type.
	 */
	public void checkEncodingType(Schema.Type encodingType) throws ConversionConnectException {
		if (encodingType != this.encodingType) {
			throw new ConversionConnectException("Logical Type " + logicalName + " must be encoded as "
					+ this.encodingType + "; " + "instead, found " + encodingType);
		}
	}
	...
	/**
	 * Build a BigQuery field for the given Kafka Connect schema. Subclasses may
	 * override to customize precision, scale, or type.
	 *
	 * @param schema
	 *            the Kafka Connect schema of the logical field
	 * @param fieldName
	 ...
	 *            the name of the field
	 * @param convertStruct
	 *            a function that converts the schema and field name into a
	 *            Field.Builder.
	 * @return a {@link Field.Builder} initialized for this logical type
	 */
	public Field.Builder getFieldBuilder(Schema schema, String fieldName,
	 ...
	 *
	 * @param kafkaConnectObject
	 *            the kafkaConnectObject
