...
public class KafkaDataBuilder {
	...
	/**
	 * This is a marker variable for methods necessary to keep original sink record
	 * metadata. These methods in SinkRecord class are available only since Kafka
	 * Connect API version 3.6.
	 */
	private static boolean KAFKA_CONNECT_API_POST_3_6;

	/**
	 * This variable determines if the original variable or the mutated variable
	 * should be used when running in a post 3.6 Kafka.
	 */
	...
	static {
		...
		try {
			MethodHandles.lookup().findVirtual(SinkRecord.class, "originalTopic", MethodType.methodType(String.class));
			MethodHandles.lookup().findVirtual(SinkRecord.class, "originalKafkaPartition",
					MethodType.methodType(Integer.class));
			MethodHandles.lookup().findVirtual(SinkRecord.class, "originalKafkaOffset",
	 ...
	 *
	 * @param kafkaDataFieldName
	 *            The configured name of Kafka Data Field
	 * @return Field of Kafka Data, with definitions of kafka topic, partition,
	 *         offset, and insertTime.
	 */
	public static Field buildKafkaDataField(String kafkaDataFieldName) {
		...
		Field partitionField = com.google.cloud.bigquery.Field.of(KAFKA_DATA_PARTITION_FIELD_NAME,
		...
		Field.Builder insertTimeBuilder = com.google.cloud.bigquery.Field
		...
		return Field.newBuilder(kafkaDataFieldName, LegacySQLTypeName.RECORD, topicField, partitionField, offsetField,
				insertTimeField).setMode(com.google.cloud.bigquery.Field.Mode.NULLABLE).build();
	}
	 ...
	 *
	 * @param kafkaConnectRecord
	 *            Kafka sink record to build kafka data from.
	 * @return HashMap which contains the values of kafka topic, partition, offset,
	 *         and insertTime.
	 */
	 ...
	 *            Kafka sink record to build kafka data from.
	 * @return HashMap which contains the values of kafka topic, partition, offset,
	 *         and insertTime in microseconds.
	 */
