...
public class KafkaLogicalConverters {
	 ...
	 *
	 * @see <a href=
	 *      "https://cloud.google.com/bigquery/docs/reference/standard-sql/data-types#numeric-type">BigQuery
	 *      data types</a>
	 */
	...
	public static void initialize() {
		...
		LogicalConverterRegistry.registerIfAbsent(Decimal.LOGICAL_NAME,
				new DecimalConverter(config.getDecimalHandlingMode()));
	...
	public static class DecimalConverter extends LogicalTypeConverter {
		...
		public Field.Builder getFieldBuilder(Schema schema, String fieldName,
			...
			switch (decimalHandlingMode) {
				case RECORD :
					com.google.cloud.bigquery.Field scaleField = Field.newBuilder("scale", LegacySQLTypeName.INTEGER)
							.setMode(Field.Mode.REQUIRED).build();
					com.google.cloud.bigquery.Field valueField = Field.newBuilder("value", LegacySQLTypeName.BYTES)
							.setMode(Field.Mode.REQUIRED).build();
					return com.google.cloud.bigquery.Field.newBuilder(fieldName, LegacySQLTypeName.RECORD,
					...
					if (decimalHandlingMode.sqlTypeName == LegacySQLTypeName.NUMERIC) {
						if (precision != null && precision > MAX_NUMERIC_PRECISION) {
							throw new ConversionConnectException(
									String.format("Requested precision (%s) is too high for %s type", precision,
											decimalHandlingMode.sqlTypeName));
						}
						if (scale != null && scale > MAX_NUMERIC_SCALE) {
							throw new ConversionConnectException(
									String.format("Requested scale (%s) is too large for %s type", precision,
											decimalHandlingMode.sqlTypeName));
						}
					}
					com.google.cloud.bigquery.Field.Builder builder = com.google.cloud.bigquery.Field
							.newBuilder(fieldName, decimalHandlingMode.sqlTypeName);
					if (precision != null) {
