...
/**
 * Tests that the EvolvingSourceRecordIterator functions correctly with the underlying storage
 * system.
 *
 * @param <K> the native key type.
 */
public abstract class AbstractSourceIteratorIntegrationTest<K extends Comparable<K>, N>
   ...
   *
   * @param configData the configuration data for the iterator.
   * @param offsetManager the OffsetManager for the iterator to use.d
   * @param extractor the Extractor for the iterator to use.
   * @return a configured SourceRecord iterator.
   */
  protected abstract Iterator<> getSourceRecordIterator(
      Map<String, String> configData, OffsetManager offsetManager, Extractor extractor);
   ...
   */
  protected final OffsetManager createOffsetManager() {
   ...
   * @param taskId the taskID, May be {@link #TASK_NOT_SET}.
   * @param maxTasks the number of tasks.
  ...
  private Map<String, String> createConfig(
      final String topic, final int taskId, final int maxTasks, final InputFormat inputFormat) {
   ...
   * Creates a configuration for the spcified arguments
   *
   * @param localPrefix the prefix to add to the native key.
   * @param topic the topic to write to.
   * @param taskId the taskID, May be {@link #TASK_NOT_SET}.
   * @param maxTasks the number of tasks.
   * @param inputFormat the input format to read.
  ...
  private Map<String, String> createConfig(
    ...
    final CommonConfigFragment.Setter setter =
        CommonConfigFragment.setter(configData).connector(getConnectorClass()).maxTasks(maxTasks);
  ...
  void zeroLengthFilesAreIgnoredTest(final InputFormat inputFormat) {
    ...
    final I sourceRecordIterator =
        getSourceRecordIterator(
            createConfig(topic, taskId, maxTasks, inputFormat),
            createOffsetManager(),
            TransformerFactory.getTransformer(inputFormat));
  ...
  void sourceRecordIteratorBytesTest() {
    ...
    final I sourceRecordIterator =
        getSourceRecordIterator(
            createConfig(topic, taskId, maxTasks, InputFormat.BYTES),
            createOffsetManager(),
  ...
  void sourceRecordIteratorAvroTest() throws IOException {
    ...
    final byte[] outputStream2 =
        AvroTestDataFixture.generateAvroData(numberOfRecords + 1, numberOfRecords);
    final byte[] outputStream3 =
        AvroTestDataFixture.generateAvroData(2 * numberOfRecords + 1, numberOfRecords);
    final byte[] outputStream4 =
        AvroTestDataFixture.generateAvroData(3 * numberOfRecords + 1, numberOfRecords);
    final byte[] outputStream5 =
        AvroTestDataFixture.generateAvroData(4 * numberOfRecords + 1, numberOfRecords);
    ...
    final I sourceRecordIterator =
        getSourceRecordIterator(
            configData, createOffsetManager(), TransformerFactory.getTransformer(InputFormat.AVRO));
    ...
    while (sourceRecordIterator.hasNext()) {
      ...
      seenRecords.compute(
          ...
          (k, v) -> {
            final List<Long> lst =
                v == null ? new ArrayList<>() : v; // NOPMD new object inside loop
  ...
  /**
   * Test the iterator can detect rehydration. Rehydration is when additional data items are written
   * to the data store after the iterator has reported exhaustion. The iterator must be able to
   * detect the new records ana process them.
   */
  @Test
