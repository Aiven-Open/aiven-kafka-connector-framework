...
public final class TemplateVariableRegistry {
     ...
     */
    public static final TemplateVariableRegistry STANDARD_SINK = builder().add(
                    TemplateVariable.KEY).add(TemplateVariable.TOPIC).add(TemplateVariable.PARTITION)
            .add(TemplateVariable.START_OFFSET).add(TemplateVariable.TIMESTAMP)
            .build();
    ...
    private TemplateVariableRegistry(Builder builder) {
        registeredVariables = new HashMap<>(builder.registeredVariables);
    }
    ...
    /**
     * Determins if the name is in the registry.
