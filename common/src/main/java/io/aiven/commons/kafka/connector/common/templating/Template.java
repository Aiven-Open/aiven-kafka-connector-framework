 ...
 *
 * <p>A template string is a string that has zero or more {@link TemplateVariable}s defined. For
 * example "This is the key {{ key }}, and this is the partition {{ partition:padding=true }}". To
 * construct a {@code Template} use the {@link TemplateParser}
 */
public final class Template {
   ...
   * Constructor.
   *
   * @param templatePattern The textual representation of the template.
   * @param templateParts the template pars as parsed from the templatePattern.
   */
  Template(final String templatePattern, final List<TemplatePart> templateParts) {
    this.templatePattern = templatePattern;
    this.templateParts = templateParts;
    this.variables =
        templateParts.stream()
            .filter(VariableTemplatePart.class::isInstance)
            .map(VariableTemplatePart.class::cast)
            .toList();
  }
  ...
  /**
   * Gets a list of variable names to parameters where the parameters are known to be non empty. If
   * duplicate variables appeared in the template there will be duplicates in the list.
   *
   * @return the list of variable names to parameters.
   */
  public List<VariableTemplatePart> variablesWithNonEmptyParameters() {
    return variables.stream()
        .filter(VariableTemplatePart::hasParameter)
        .collect(Collectors.toList());
  }
  ...
  public final class Bound {
    ...
    public String render() {
      ...
      templateParts.stream()
          .map(templatePart -> templatePart.render(bindings))
  ...
  public class BoundBuilder {
    ...
    /**
     * Adds all the bindings from the builder to this builder. Will overwrite any bindings with the
     * same name.
     *
     * @param builder the builder to copy bindings from.
     * @return this.
     */
     ...
     *
     * @param bound the Template.Bound to copy from.
     * @return this
     */
     ...
     *
     * @param name the name to bind.
     * @param binding the function to bind.
     * @throws IllegalArgumentException if the variable is not in the template.
     * @return this
     */
    public BoundBuilder bind(final String name, final Function<Parameter, String> binding) {
      ...
      if (variableNames.contains(name)) {
        ...
        throw new IllegalArgumentException(
            String.format(
                "Parameter '%s' does not exist in the template '%s'", name, templatePattern));
      }
    }
    ...
    public Bound build() {
      if (LOGGER.isInfoEnabled()) {
        List<String> missingBindings =
            variableNames.stream().filter(name -> !bindings.containsKey(name)).toList();
   ...
   *
   * <p>Where an {@link Bound} generates strings by filling in the variables in a template, the
   * {@link Extractor} does the opposite:
   *
   * <pre>
   ...
   * </pre>
   *
   * The intention is that applying the map back to the template will yield the original string, but
   * the round-trip is not always exact.
   *
   * <ul>
   *   <li>Rendering a string uses a supplier and the extractor returns a fixed string.
   *   <li>Variable parameters are currently ignored (to be determined).
   * </ul>
   */
  public final class Extractor {
    ...
    private Extractor() {
      ...
      templateParts.stream()
          .map(templatePart -> templatePart.extract(captureGroups))
     ...
     *
     * @param input A string to extract variables from.
     * @return An immutable map of key-value pairs extracted from the input string, corresponding to
     *     the template variable names.
     * @throws IllegalArgumentException If the input string does not match the template.
     */
