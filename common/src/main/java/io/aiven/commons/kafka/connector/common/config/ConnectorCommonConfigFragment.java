...
public class ConnectorCommonConfigFragment extends ConfigFragment {
  ...
  public static final String RELAX_SCHEMES =
      "io.aiven.commons.kafka.connector.common.config.RELAX_SCHEMES";

  /** the schema registry URL */
  static final String SCHEMA_REGISTRY_URL = "schema.registry.url";

  /**
   * The flag to enable the schema registry. The schema registry enablement flag. Defaults to {@code
   * true} if {@link #SCHEMA_REGISTRY_URL} is set.
   */
  static final String SCHEMA_REGISTRY_ENABLE = "schema.registry.enable";

  /** The value converter registry URL. Defaults to the {@link #SCHEMA_REGISTRY_URL}. */
  static final String VALUE_CONVERTER_SCHEMA_REGISTRY_URL = "value.converter.schema.registry.url";

  /**
   * The flag to enable the value converter schema registry. Defaults to {@link
   * #SCHEMA_REGISTRY_ENABLE} if {@link #VALUE_CONVERTER_SCHEMA_REGISTRY_URL} is set.
   */
  static final String VALUE_CONVERTER_SCHEMA_REGISTRY_ENABLE =
  ...
  /**
   * The flag to enable the key converter schema registry. Defaults to {@link
   * #SCHEMA_REGISTRY_ENABLE} if {@link #KEY_CONVERTER_SCHEMA_REGISTRY_URL} is set.
   */
  ...
  public static ConfigDef update(final ConfigDef configDef) {
    ...
    configDef
        .define(
            ExtendedConfigKey.builder(SCHEMA_REGISTRY_URL)
                .validator(schemaUrlValidator)
                .group(COMMON_GROUP)
   ...
   *
   * @param configMap the change tracking map to process.
   */
    ...
    if (configMap.get(SCHEMA_REGISTRY_URL) != null) {
      ...
      if (configMap.get(VALUE_CONVERTER_SCHEMA_REGISTRY_URL) == null) {
        ...
        if (configMap.get(VALUE_CONVERTER_SCHEMA_REGISTRY_ENABLE) == null) {
          configMap.override(
              VALUE_CONVERTER_SCHEMA_REGISTRY_ENABLE, configMap.get(SCHEMA_REGISTRY_ENABLE));
        }
      } else {
        if (configMap.get(VALUE_CONVERTER_SCHEMA_REGISTRY_ENABLE) == null) {
      ...
      if (configMap.get(KEY_CONVERTER_SCHEMA_REGISTRY_URL) == null) {
        ...
        if (configMap.get(KEY_CONVERTER_SCHEMA_REGISTRY_ENABLE) == null) {
          configMap.override(
              KEY_CONVERTER_SCHEMA_REGISTRY_ENABLE, configMap.get(SCHEMA_REGISTRY_ENABLE));
        }
      } else {
        if (configMap.get(KEY_CONVERTER_SCHEMA_REGISTRY_ENABLE) == null) {
      ...
      if (configMap.get(VALUE_CONVERTER_SCHEMA_REGISTRY_ENABLE) == null) {
        configMap.override(
            VALUE_CONVERTER_SCHEMA_REGISTRY_ENABLE,
            configMap.get(VALUE_CONVERTER_SCHEMA_REGISTRY_URL) != null);
      }

      if (configMap.get(KEY_CONVERTER_SCHEMA_REGISTRY_ENABLE) == null) {
        configMap.override(
            KEY_CONVERTER_SCHEMA_REGISTRY_ENABLE,
  ...
  public void validate(Map<String, ConfigValue> configMap) {
    ...
    if (schemaEnable.value() == Boolean.TRUE && schemaReg.value() == null) {
      registerIssue(
          ...
          schemaEnable.value(),
          String.format(
              "%s may not be 'true' if %s is unset", SCHEMA_REGISTRY_ENABLE, SCHEMA_REGISTRY_URL));
    }

    /* Value converter schema processing */
    if (valueEnable.value() != null
        && valueEnable.value() == Boolean.TRUE
        && valueReg.value() == null) {
      registerIssue(
          configMap,
          VALUE_CONVERTER_SCHEMA_REGISTRY_ENABLE,
          valueEnable.value(),
          String.format(
              "%s may not be 'true' if both %s and %s are unset",
              VALUE_CONVERTER_SCHEMA_REGISTRY_ENABLE,
  ...
  /**
   * Gets the schema registry enabled flag. If {@code true} the system should utilize the schema
   * registry.
   *
  ...
  /**
   * Gets the value converter schema registry URL. May be {@code null} if not set and {@link
   * #getSchemaRegistryUrl} returns null.
   *
  ...
  /**
   * Gets the value converter schema registry enabled flag. If {@code true} the system should
   * utilize the value converter schema registry.
   *
  ...
  /**
   * Gets the key converter schema registry URL. May be {@code null} if not set and {@link
   * #getSchemaRegistryUrl} returns null.
   *
  ...
  /**
   * Gets the key converter schema registry enabled flag. If {@code true} the system should utilize
   * the key converter schema registry.
   *
  ...
  public static class Setter extends AbstractFragmentSetter<Setter> {
     ...
     * Sets the schema registry URL.
     *
     * @param schemaRegistryUri the schema registry URL
     * @return this.
     */
     ...
     *
     * @param enableSchemaRegistry the state of the schema registry flag.
     * @return this
     */
     ...
     *
     * @param registryUrl the value converter schema registry URL
     * @return this.
     */
     ...
     * Sets the value converter schema registry URL.
     *
     * @param registryUri the value converter schema registry URL
     * @return this.
     */
     ...
     *
     * @param enableRegistry the state of the value converter schema registry flag.
     * @return this
     */
     ...
     *
     * @param registryUrl the key converter schema registry URL
     * @return this.
     */
     ...
     * Sets the key converter schema registry URL.
     *
     * @param registryUri the key converter schema registry URL
     * @return this.
     */
     ...
     * Sets the value converter schema registry flag.
     *
     * @param enableRegistry the state of the key converter schema registry flag.
     * @return this
     */
     ...
     *
     * @param compressionType the compression type expected for I/O to storage.
     * @return this
     */
