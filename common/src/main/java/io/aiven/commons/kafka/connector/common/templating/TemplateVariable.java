...
public final class TemplateVariable {
	...
	private static final ParameterDescriptor PADDING_DESCRIPTOR = ParameterDescriptor.builder("padding")
			.validator(ConfigDef.ValidString.in("true", "false"))
			.description("Specifies that the value should be left padded").build();
	/**
	 * The standard key definition
	 */
	public static final TemplateVariable KEY = builder("key").description("The key from the Kafka source record")
			.build();
	/**
	 * the standard topic definition
	 */
	public static final TemplateVariable TOPIC = builder("topic").description("The topic the message was read from")
			.build();
	/**
	 * the standard partition definition
	 */
	public static final TemplateVariable PARTITION = builder("partition").parameterDescriptor(PADDING_DESCRIPTOR)
			.description("The partition the message was read from").build();
	/**
	 * the standard start offset definition
	 */
	public static final TemplateVariable START_OFFSET = builder("start_offset").parameterDescriptor(PADDING_DESCRIPTOR)
			.build();
	/**
	 * The standard timestamp definition
	 */
	public static final TemplateVariable TIMESTAMP = builder("timestamp")
			.parameterDescriptor(ParameterDescriptor.builder("unit").required(true)
					.validator(ConfigDef.ValidString.in("yyyy", "MM", "dd", "HH"))
					.description("Specifies the format of the timestamp."))
			.description("The timestamp from the message").build();
	...
	public String example() {
		return String.format("{{ %s%s }}", name, hasParameter() ? ":" + parameterDescriptor.example() : "");
	}
	 ...
	 * 
	 * @param errStr
	 *            the error string for this template.
	 * @param template
	 *            the template the parameter was parsed from.
	 * @param parameter
		...
		if (hasParameter()) {
			...
			if (!parameter.equals(Parameter.EMPTY)) {
				if (!parameterDescriptor.getName().equals(parameter.getName())) {
					throw new ConfigException(errStr, template,
							String.format("parameter name should be '%s'", parameterDescriptor.getName()));
				}
				parameterDescriptor.validate(String.format("%s parameter '%s'", errStr, parameterDescriptor.getName()),
						template, parameter.getValue());
			}
		}
	}
	...
	public static class Builder {
		 ...
		 * 
		 * @param builder
		 *            the parameter descriptor builder.
		 * @return this
		 */
